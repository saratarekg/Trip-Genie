  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div >
        ...
        >
          {activities.map((act) => (
            <div key={act.id} className="flex-shrink-0 w-full sm:w-1/2 md:w-1/3 rounded-lg overflow-hidden shadow-lg"
              style={{ backgroundColor: 'rgb(255, 248, 241)' }}>
              <img
                  src={Array.isArray(act.pictures) ? act.pictures[0] : act.pictures}
                  alt={act.name} className="w-full h-48 object-cover"
                />
              <div className="p-4">
                ...
                <div className="flex mb-2">
                  {Array.from({ length: 5 }, (_, i) => {
                    if (i < Math.floor(act.rating)) {
                      ...
                      return <span key={i} className="text-yellow-400">☆</span>; // Or use a different half-star symbol
                    } else {
                      // Empty star
                ...
                <div className="flex justify-between items-center">
                  ...
                  <button className="bg-orange-500 text-white px-4 py-2 rounded text-sm">DETAILS</button>
                </div>
              </div>
            </div>
          ))}
