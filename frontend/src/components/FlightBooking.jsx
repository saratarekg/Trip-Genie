...
function BookingPage() {
  ...
  const getCurrencyCode = async () => {
    try {
      ...
      const response = await fetch("http://localhost:4000/tourist/currencies/code", {
        method: "GET",
        headers: {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (!data.data || data.data.length === 0) {
        ...
        setCurrentPage(1)
        const prices = data.data.map(flight => parseFloat(flight.price.total))
    ...
    filtered.sort((a, b) => {
      ...
      switch(sortBy) {
        ...
        case "departure":
          ...
          break
        case "arrival":
          aValue = new Date(a.itineraries[0].segments[a.itineraries[0].segments.length - 1].arrival.at)
          bValue = new Date(b.itineraries[0].segments[b.itineraries[0].segments.length - 1].arrival.at)
          break
        default:
          aValue = parseFloat(a.price.total)
          bValue = parseFloat(b.price.total)
                ...
                <CardContent className="p-3">
                  <div className="flex flex-col h-full">
                    ...
                    <div className="text-sm text-muted-foreground space-y-1">
                      ...
                      <div className="flex items-center gap-2">
                        <Calendar className="h-4 w-4" />
                        <span>Arrival: {formatDateTime(flight.itineraries[0].segments[flight.itineraries[0].segments.length - 1].arrival.at)}</span>
                      </div>
                      {flight.itineraries[1] && (
                        <>
                          ...
                          <div className="flex items-center gap-2">
                            <Calendar className="h-4 w-4" />
                            <span>Return Arrival: {formatDateTime(flight.itineraries[1].segments[flight.itineraries[1].segments.length - 1].arrival.at)}</span>
                          </div>
                        </>
                      )}
                    </div>
                        ...
                        <DialogContent className="sm:max-w-[425px]">
                          ...
                          {selectedFlight && (
                            <div  className="mt-4">
                              ...
                              <Button className="mt-4 w-full bg-blue-900 hover:bg-blue-800 text-white" onClick={handleBookNow}>
                                Book Now
                              </Button>
                            </div>
                          )}
