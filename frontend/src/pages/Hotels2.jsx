...
export default function HotelSearch() {
  ...
  const searchRegionId = async () => {
    const url = `https://${API_HOST}/v2/regions?query=${encodeURIComponent(city)}&domain=AR&locale=es_AR`;
      ...
      {searchResults.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
          {searchResults.map((hotel) => (
            <Card key={hotel.id} className="cursor-pointer" onClick={() => handleHotelSelect(hotel)}>
              <CardHeader>
              ...
              <CardContent>
                ...
                />
                <p className="text-sm text-gray-600">{hotel.neighborhood?.name}</p>
      ...
      {selectedHotel && hotelDetails && (
        <Card className="w-full max-w-4xl mx-auto mb-8">
          <CardHeader>
            ...
            <CardDescription>{hotelDetails.summary.location.address.addressLine}</CardDescription>
          </CardHeader>
          <CardContent>
            <img 
              src={hotelDetails.propertyGallery.images[0]?.image.url || '/placeholder.svg'}
              alt={hotelDetails.summary.name}
              className="w-full h-64 object-cover rounded-lg mb-4"
            />
            <p className="text-sm text-gray-600 mb-4">{hotelDetails.summary.tagline}</p>
            <h3 className="text-lg font-semibold mb-2">Amenities</h3>
            <div className="flex flex-wrap gap-2 mb-4">
              {hotelDetails.amenities.topAmenities.items.map((amenity, index) => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {roomOffers.map((room) => (
                <Card key={room.unitId}>
                  ...
                  <CardContent>
                    <p className="text-sm text-gray-600 mb-2">{room.ratePlans[0]?.priceDetails[0]?.price.lead.formatted}</p>
                    <div className="flex items-center">
                      <UsersIcon className="w-5 h-5 mr-2" />
                      <span>Max occupancy: {room.ratePlans[0]?.priceDetails[0]?.availability?.minRoomsLeft || 'N/A'}</span>
                    </div>
                  </CardContent>
                  <CardFooter>
