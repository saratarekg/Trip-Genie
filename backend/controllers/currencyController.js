...
const populateCurrencies = async (req, res) => {
  try {
    ...
    for (const currency of predefinedCurrencies) {
      ...
      if (!existingCurrency) {
        // If the currency doesn't exist, create a new document
        const newCurrency = new Currency(currency);
        await newCurrency.save();
        console.log(`Added ${currency.code} to the database`);
      } else {
        console.log(`${currency.code} already exists in the database`);
      }
    }
  ...
  try {
    ...
    if (!baseRate || !targetRate) {
      return res
        .status(404)
        .json({
          message: "Exchange rate not available for one or both currencies.",
        });
    }
...
const getSupportedCurrencies = async (req, res) => {
  try {
    ...
    if (!currencies || currencies.length === 0) {
      return res
        .status(404)
        .json({ message: "No currencies found in the database." });
    }
    ...
    res
      .status(500)
      .json({ message: "Error fetching currencies from the database." });
  }
};
