...
app.post("/create-booking-session", async (req, res) => {
  try {
    const { items, currency, quantity, returnLocation, selectedDate } = req.body;
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "payment",
      success_url: `${returnLocation}/?success=true&session_id={CHECKOUT_SESSION_ID}&quantity=${quantity}&selectedDate=${selectedDate}`,
