...
historicalPlacesSchema.statics.filterByTag = async function (types, periods) {
    ...
    if ((types === undefined || types === null || types.length === 0) && (periods === undefined || periods === null || periods.length === 0)) {
        ...
        const array = Array.isArray(periods)
      ? periods
      : periods.split(","); // Ensure it's an array
        historicalTags = await HistoricalTag.find({ period: { $in: array } });
    }
    else if (periods === undefined || periods === null || periods.length === 0) {
        const array = Array.isArray(types)
      ? types
      : types.split(","); // Ensure it's an array
        ...
        const array2 = Array.isArray(periods)
        ? periods
        : periods.split(","); // Ensure it's an array
        historicalTags = await HistoricalTag.find({ type: { $in: array }, period: { $in: array2 } });
    }
